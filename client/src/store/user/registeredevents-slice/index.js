...
export const getUserRegisteredEvents = createAsyncThunk(
  ...
  async (userId) => {
    ...
    const result = await axios.get(
      `http://localhost:5000/api/user/registeredevents/get/${userId}`
    );
...
export const registerEvent = createAsyncThunk(
  ...
  async ({userId, eventId}) => {
    ...
    const result = await axios.post(
      "http://localhost:5000/api/user/registeredevents/add",
      { userId, eventId },
      {
        headers: {
...
export const leaveEvent = createAsyncThunk(
  ...
  async ({ userId, eventId }) => {
    ...
    const result = await axios.delete(
      `http://localhost:5000/api/user/registeredevents/leave/${userId}/${eventId}`,
      {
        headers: {
          "Content-Type": "application/json",
        },
